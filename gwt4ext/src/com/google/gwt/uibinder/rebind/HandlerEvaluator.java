...
class HandlerEvaluator {
     ...
     * 
     * http://docs.google.com/Doc?docid=0AQfnKgX9tAdgZGZ2cTM5YjdfMmQ4OTk0eGhz&hl=
     * en
     * 
     * But that needs to wait for a refactor to get most of this stuff out of
     * here and into com.google.gwt.uibinder.rebind.model
     */
     ...
     * 
     * @param ownerClass
     *            a descriptor of the UI owner class
     * @param logger
     *            the logger for warnings and errors
     * @param oracle
     *            the type oracle
     */
    HandlerEvaluator(OwnerClass ownerClass, MortalLogger logger, TypeOracle oracle, boolean useLazyWidgetBuilders) {
    ...
    public void run(IndentedWriter writer, FieldManager fieldManager, String uiOwner) throws UnableToCompleteException {
        ...
        for (JMethod method : ownerClass.getUiHandlers()) {
            ...
            for (String objectName : annotation.value()) {
                ...
                if (fieldWriter == null) {
                    logger.die(("Method '%s' can not be bound. You probably missed ui:field='%s' " + "in the template."),
                                    boundMethod, objectName);
                }

                // Retrieves the "add handler" method in the object.
                JMethod addHandlerMethodType = getAddHandlerMethodForObject(fieldWriter.getInstantiableType(),
                                handlerType);
     ...
     * @param handlerType
     *            the handler we want to create
     * @param eventType
     *            the event associated with the handler
     * @param boundMethod
     *            the method bound in the handler
     */
    protected void writeHandler(IndentedWriter writer, String uiOwner, String handlerVarName, JClassType handlerType,
                    JClassType eventType, String boundMethod) throws UnableToCompleteException {

        // Retrieves the single method (usually 'onSomething') related to all
        // handlers. Ex: onClick in PinchChangedDragStateHandler, onBlur in
        // BlurHandler ...
        ...
        writer.newline();
        writer.write("final %1$s %2$s = new %1$s() {", handlerType.getParameterizedQualifiedSourceName(),
                        handlerVarName);
        writer.indent();
     ...
     * @param handlerVarName
     *            the name of the handler variable
     * @param addHandlerMethodName
     *            the "add handler" method name associated with the object
     * @param objectName
     *            the name of the object we want to tie the handler
     */
    void writeAddHandler(IndentedWriter writer, FieldManager fieldManager, String handlerVarName,
                    String addHandlerMethodName, String objectName) {
        if (useLazyWidgetBuilders) {
            fieldManager.require(objectName).addStatement("%1$s.%2$s(%3$s);", objectName, addHandlerMethodName,
                            handlerVarName);
        } else {
     ...
     * 
     * <p>
     * Output an error in case more than one method match the conditions
     * described above.
     * </p>
     * 
     * <pre>
     ...
     * @param handlerType
     *            the handler type we want to check in the object
     * 
    ...
    private JMethod getAddHandlerMethodForObject(JClassType objectType, JClassType handlerType)
        ...
        for (JMethod method : objectType.getInheritableMethods()) {
            ...
            if (method.getReturnType() == handlerRegistrationJClass
                            || (method.getReturnType().isClassOrInterface() != null && classImplementsInterface(method
                                            .getReturnType().isClassOrInterface(), handlerRegistrationJClass))) {
                ...
                if (handlerType.equals(subjectHandler)) {
                    ...
                    if (handlerMethod != null) {
                        logger.die(("This handler cannot be generated. Methods '%s' and '%s' are "
    ...
    private JClassType getHandlerForEvent(JClassType eventType) {
        ...
        if (method == null) {
            logger.warn("Method 'getAssociatedType()' could not be found in the event '%s'.", eventType.getName());
            return null;
        }

        JType returnType = method.getReturnType();
        if (returnType == null) {
            logger.warn("The method 'getAssociatedType()' in the event '%s' returns void.", eventType.getName());
            return null;
        }

        JParameterizedType isParameterized = returnType.isParameterized();
        if (isParameterized == null) {
            logger.warn("The method 'getAssociatedType()' in '%s' does not return Type<? extends EventHandler>.",
                            eventType.getName());
            return null;
        }
